...
;;all sat/ unsat/ should work, including cnf_42 works, which is just extremely slow.
;Break 34 [68]> (time (sat? (first d) (second d)))
;Real time: 811.5965 sec.
;Run time: 807.92 sec.
;Space: 342069040 Bytes
;GC: 332, GC time: 2.501217 sec.
;;
...
;returns true if any clause is flase
(defun forward-checking (delta opsAssign)
  (let* ((head (first delta)))
    (cond 
      ((null delta) nil)
      ((forward-checking-clause head opsAssign) t)
      (t (forward-checking (rest delta) opsAssign)))))

;returns true only if all literals in caluse exist in assign, which means the clause is false
(defun forward-checking-clause (clause opsAssign)
  (let* ((literal (first clause)))
    (cond 
      ((null clause) t)
      ((<= (count literal opsAssign) 0) nil)
      (t (forward-checking-clause (rest clause) opsAssign)))))

;returns the opposite value of this list
(defun ops-list (assign)
  (let* ((element (first assign)))
    (cond
      ((null assign) nil)
      (t (cons (- 0 element) (ops-list (rest assign)))))))
